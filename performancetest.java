...
public static void run(boolean details, int iterations, int seed, int aChunkX, int aChunkZ)  //sortof the core of the new run function in worldgencontainer
    ...
    static final boolean debugStones = true;
    static final double sizeConversion[] = { 1, 1.333333, 2, 2, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4 }; // Bias the sizes towards skinnier boulders, ie more "shafts" than dikes or sills. 
    
    static void GT_stones( int x, int z, int aChunkX, int aChunkZ, int stoneSize, boolean details )
    {
        mSize = stoneSize/4;  //For historic reasons
        XSTR stoneRNG = new XSTR( Math.abs(x) + Math.abs(z) + 46);

        // Locate the stoneseed XYZ
        int tX = x*16 + stoneRNG.nextInt(16);
        ...
        int tMinX = tX-(int)(mSize/xSize-1.0); 
        int tMaxX = tX+(int)(mSize/xSize+2.0);
        int tMinY = tY-(int)(mSize/ySize-1.0);
        int tMaxY = tY+(int)(mSize/ySize+2.0);
        int tMinZ = tZ-(int)(mSize/zSize-1.0);
        int tMaxZ = tZ+(int)(mSize/zSize+2.0);
        ...
        if (debugStones) System.out.println(  //Sub back in GT_Log!!
            "tX=" + tX +
            " tY=" + tY +
            " tZ=" + tZ +
            " mSize=" + mSize +
        ...
        double rightHandSide = mSize*mSize;  //Precalc the right hand side 
        for( int iY = tMinY; iY < tMaxY; iY++) {  // Do placement from the bottom up layer up.  Maybe better on cache usage?
            ...
            for( int iX = wX; iX < eX; iX++) {
                ...
                xCalc = xCalc * xCalc;
                result = yCalc + xCalc;
                if( result > rightHandSide ) { // Again, if X and Y is larger than the RHS, skip to the next value
                    continue;
                }
                for( int iZ = sZ; iZ < nZ; iZ++ ) {
                    double zCalc = ( (double)(iZ-tZ)*zSize );
                    zCalc = zCalc * zCalc;
                    result = zCalc + xCalc + yCalc;
                    if( result > rightHandSide ) {
                        continue;
                    } else {
                        // Yay! We can actually place a block now.
                        setOreBlock(1, iX, iY, iZ, 42, false, false);
                    }
                }
            }
        }
        ...
        if (details){
            for( int depth = (tMaxY); depth > (tMinY-2); depth--){    
                printChunkSlice(Chunk, depth);
            }
        }
    }
...
//        testclass.executeWorldgenNew(true, 1, -10, 32, 32);
    ...
    //    testclass.run(false, 1, 2, 16, 64);
        testclass.initChunk(testclass.Chunk);
        testclass.GT_stones( 2, 2, 1*16, 0*16,  32*4, false);
        testclass.GT_stones( 2, 2, 1*16, 1*16,  32*4, false);
        testclass.GT_stones( 2, 2, 2*16, 1*16,  32*4, false);
        testclass.GT_stones( 2, 2, 1*16, 2*16,  32*4, false);
        testclass.GT_stones( 2, 2, 2*16, 0*16,  32*4, false);
        testclass.GT_stones( 2, 2, 2*16, 2*16,  32*4, false);
        testclass.GT_stones( 2, 2, 1*16, 3*16,  32*4, false);
        testclass.GT_stones( 2, 2, 2*16, 3*16,  32*4, true);
